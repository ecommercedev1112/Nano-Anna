...
class DeferredMedia extends HTMLElement {
  constructor() {
    ...
    // Initialize Intersection Observer on mobile devices
  ...
  initIntersectionObserver() {
    // Check if we're within .product__media-list.slider--mobile
    if (!this.closest('.product__media-list.slider--mobile')) return;
      ...
      (entries, observer) => {
        entries.forEach((entry) => {
          ...
          if (entry.isIntersecting) {
            ...
            if (this.getAttribute('loaded')) {
              // If the deferred element is a video, attempt to play it
              if (deferredElement && deferredElement.nodeName == 'VIDEO') {
                deferredElement.play().catch((e) => console.error('Video play failed', e));
              }
            } else {
              // If the content is not loaded, load it
              this.loadContent(false); // Load content without focusing
            }
          } else if (deferredElement && deferredElement.nodeName == 'VIDEO') {
            // Pause the video if it's not intersecting (out of view)
